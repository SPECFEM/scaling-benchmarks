...
if [ "$forward" == "" ]; then echo "usage: ./mk_titan.sh forward(==1/==2 for kernel binaries)"; exit 1; fi
...
## cross-compiling:
host=craype-istanbul #craype-target-native # craype-target-local_host # craype-target-native # xtpe-istanbul
default=craype-interlagos
