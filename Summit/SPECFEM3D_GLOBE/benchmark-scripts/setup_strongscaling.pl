...
$masterdir = "${HOMEDIR}/benchmark-scripts";
...
#for($j=1;$j<=$maxCPUs;$j=$j+$incr){
  ...
  # cleans OUTPUTS/
  #-----------------
  #if(-e "$masterdir/OUTPUTS/OUTPUT_FILES.$cpus"){system("rm -rf $masterdir/OUTPUTS/OUTPUT_FILES.$cpus");}
  ...
  # copy binaries
  #system("cp -p $masterdir/bin/xmeshfem3D $newdir/bin/");
  #system("cp -p $masterdir/bin/xspecfem3D $newdir/bin/");

  # copy scripts
  system("cp -p $masterdir/$jobscript $newdir/$jobscript");

  # copy backup of Par_file,STATIONS and src
  system("cp -p $masterdir/DATA/Par_file $newdir/DATA/");
  system("cp -p $masterdir/DATA/STATIONS $newdir/DATA/");
  system("cp -p $masterdir/DATA/CMTSOLUTION $newdir/DATA/");
  ...
  if( $GPU eq "GPU"){
    ...
    if( $PBS == 1 ){
      # PBS
      print "  PBS scheduler\n";
      system("sed -i 's/^#PBS -l nodes=.*/#PBS -l nodes=$cpus/' $jobscript");
      #system("sed -i 's/^#PBS -l feature=.*/#PBS -l feature=gpu/' $jobscript");
      ...
      system("sed -i 's/^#BSUB -nnodes .*/#BSUB -nnodes $nodes/' $jobscript");
      system("sed -i 's/jsrun -n \$NPROC/jsrun -n$cpus -r$ppn -g1 -a1 -c1/' $jobscript");
      ...
      system("sed -i 's/^#SBATCH --nodes=.*/#SBATCH --nodes=$cpus/' $jobscript");
      system("sed -i 's/^#SBATCH --ntasks=.*/#SBATCH --ntasks=$cpus/' $jobscript");
      system("sed -i 's/^#SBATCH --ntasks-per-node=.*/#SBATCH --ntasks-per-node=1/' $jobscript");
      #system("sed -i 's/^srun -n \$NPROC/srun -n \$NPROC -N 1/' $jobscript");
      system("sed -i 's/^srun -n \$NPROC/srun /' $jobscript");
    ...
    if( $PBS == 1 ){
      # PBS
      print "  PBS scheduler\n";
      system("sed -i 's/^#PBS -l nodes=.*/#PBS -l nodes=$nodes/' $jobscript");
      #system("sed -i 's/^#PBS -l feature=.*/##PBS -l feature=gpu/' $jobscript");
      system("sed -i 's/^aprun -n \$NPROC/aprun -n \$NPROC/' $jobscript");
    }elsif( $PBS == 2 ){
      # LSF
      print "  LSF scheduler\n";
      system("sed -i 's/^#BSUB -nnodes .*/#BSUB -nnodes $nodes/' $jobscript");
      system("sed -i 's/jsrun -n \$NPROC/jsrun -n$cpus -r$ppn -g0 -a1 -c1/' $jobscript");
      #system("sed -i 's/jsrun -n \$NPROC .\/bin\/xspecfem3D/jsrun -n$ppn -g0 -a1 -c1 .\/bin\/xspecfem3D/' $jobscript");
    }else{
      # SLURM
      print "  Slurm scheduler\n";
      system("sed -i 's/^#SBATCH --nodes=.*/#SBATCH --nodes=$nodes/' $jobscript");
      system("sed -i 's/^#SBATCH --ntasks=.*/#SBATCH --ntasks=$cpus/' $jobscript");
      system("sed -i 's/^#SBATCH --ntasks-per-node=.*/#SBATCH --ntasks-per-node=$ppn/' $jobscript");
      system("sed -i 's/^srun -n \$NPROC/srun -n \$NPROC/' $jobscript");
    }
  }
